...
*/
	...
	if(isset($_POST["submit"])) {
		...
		if (file_exists($target_file_new)) {
                                ...
                                echo "alert('" .$_SESSION['message']."'); window.location.href='http://br-t2-jci.sfcrjci.org/publish.php';";
                                echo "</script>\n";
                                exit;
			...
			if (move_uploaded_file($_FILES["fileToUpload"]["tmp_name"], $target_file_new)) {
				...
				if(isset($_POST["submit"])) {
					 $dbConnect = 'mysql:dbname=br_t2_jci;host=localhost';
                        ...
                        $title = $_POST['journalTitle'];
                        //validation for title input
                        $notes = $_POST['abstract'];
                        //validation for startDate input
						$time = date("Y-m-d H:i:s", time());
                        $sql = $dbConnect->prepare("INSERT INTO `journal`(`journalID`, `categoryID`, `datePublished`, `title`, `abstract`, `fileLocation`, `displayed`)VALUES(null, null, :date, :title, :notes, :fileLocation, 1)");
				  ...
				  echo "<script language='javascript'>\n";
				  echo "alert('Upload of" .$_SESSION['message']." successful!'); window.location.href='http://br-t2-jci.sfcrjci.org/publish.php'";
				  echo "</script>\n";

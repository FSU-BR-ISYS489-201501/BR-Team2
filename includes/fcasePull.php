...
Description:
	This page controlls the cases displayed to the public based on the cases display status.
*/
session_start(); 
...
//Selects all cases information where the displayed status is set to 1
$journal = $dbConnect->prepare("SELECT * FROM `finalCase` WHERE `displayed`=:check ORDER BY `datePublished` DESC"); //Selects all cases currently set to display and orders by most recent date published.
$journal->bindValue(":check", $check);
$journal->execute();

//echos all cases file locations, title and date based on its dipsplayed status
